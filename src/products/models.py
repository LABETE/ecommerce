...
class Variation(models.Model):
    ...
    def get_html_price(self):
        if self.sale_price is not None:
            html_text = "<span class='sale-price'>{0}</span> <span class='og-price'>{1}</span>".format(self.sale_price, self.price)
        else:
            html_text = "<span class='price'>{0}</span>".format(self.price)
...
def image_upload_to(instance, filename):
    ...
    new_filename = "{0}-{1}.{2}".format(basename, instance.id, file_extension)
    return "products/{0}/{1}".format(slug, new_filename)
...
def image_upload_to_featured(instance, filename):
    ...
    basename, file_extension = filename.split(".")
    new_filename = "{0}-{1}.{2}".format(basename, instance.id, file_extension)
    return "products/{0}/featured/{1}".format(slug, new_filename)
